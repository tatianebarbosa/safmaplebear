...
class DataProcessingService:
    ...
    def load_official_data(self, force_reload: bool = False) -> Tuple[List[OfficialSchool], List[OfficialUser]]:
        ...
        try:
            ...
            schools_df = blob_service.read_excel_file('Franchising_oficial.xlsx')
            schools = self._process_schools_dataframe(schools_df)
            
            # Load users from CSV
            users_df = blob_service.read_csv_file('usuarios_public.csv')
            users = self._process_users_dataframe(users_df)
    ...
    def _process_schools_dataframe(self, df: pd.DataFrame) -> List[OfficialSchool]:
        """Process schools dataframe into OfficialSchool objects"""
        schools = []
        
        for _, row in df.iterrows():
            try:
                school_id = str(row.get('ID da Escola', '')).strip()
                if not school_id or school_id == 'nan':
                    continue
                
                # Get custom limit or default to 2
                license_limit = self._school_limits_cache.get(school_id, 2)
                
                school = OfficialSchool(
                    id=school_id,
                    name=str(row.get('Nome da Escola', '')).strip(),
                    status=str(row.get('Status da Escola', '')).strip(),
                    cluster=str(row.get('Cluster', '')).strip(),
                    carteira_saf=str(row.get('Carteira SAF', '')).strip(),
                    cnpj=str(row.get('CNPJ', '')).strip(),
                    address=str(row.get('Logradouro Escola', '')).strip(),
                    neighborhood=str(row.get('Bairro Escola', '')).strip(),
                    cep=str(row.get('CEP Escola', '')).strip(),
                    city=str(row.get('Cidade da Escola', '')).strip(),
                    state=str(row.get('Estado da Escola', '')).strip(),
                    region=str(row.get('Região da Escola', '')).strip(),
                    phone=str(row.get('Telefone de Contato da Escola', '')).strip(),
                    email=str(row.get('E-mail da Escola', '')).strip(),
                    license_limit=license_limit,
                    used_licenses=0  # Will be calculated later
                )
                
                schools.append(school)
                
            except Exception as e:
                print(f"Erro ao processar escola {row}: {str(e)}")
                continue
        
        return schools
    
    def _process_users_dataframe(self, df: pd.DataFrame) -> List[OfficialUser]:
        """Process users dataframe into OfficialUser objects"""
        users = []
        ...
        for _, row in df.iterrows():
            try:
                ...
                school_id = str(row.get('Escola ID', '')).strip()
                status_licenca = str(row.get('Status Licença', '')).strip()
                
                # Check for user overrides
                has_canva = StatusLicencaHelper.has_canva_license(status_licenca)
                override_key = f"{school_id}:{email}"
                if override_key in self._user_overrides_cache:
                    has_canva = self._user_overrides_cache[override_key].get('has_canva', has_canva)
                
                user = OfficialUser(
                    name=str(row.get('Nome', '')).strip(),
                    email=email,
                    role=str(row.get('Função', 'Estudante')).strip(),
                    school_name=str(row.get('Escola', '')).strip(),
                    school_id=school_id,
                ...
                print(f"Erro ao processar usuário {row}: {str(e)}")
                continue
    ...
    def _log_audit_action(self, action: str, license_action: LicenseAction, actor: str, payload: Dict):
        ...
        log_entry = AuditLogEntry(
            ...
            school_name=school.name if school else license_action.school_id,
            actor=actor,
            payload=payload
        )
        
        blob_service.append_audit_log(log_entry.to_dict())
