...
const AgendaSafPage = () => {
  ...
  const { data: monthEvents = [], isLoading, isFetching, refetch, error } = useQuery<CalendarEvent[]>({
  ...
  const eventsByDay = useMemo(() => {
    ...
    monthEvents.forEach((event) => {
      try {
        const dayKey = format(parseISO(event.dataInicio), "yyyy-MM-dd");
    ...
    map.forEach((list) => {
      list.sort((a, b) => {
        const aStart = Date.parse(a.dataInicio || "");
        const bStart = Date.parse(b.dataInicio || "");
  ...
  const handleMonthChange = (delta: number) => {
    const nextVisible = startOfMonth(addMonths(visibleMonth, delta));
    const clampedDay = Math.min(selectedDate.getDate(), getDaysInMonth(nextVisible));
    const nextSelected = new Date(
      nextVisible.getFullYear(),
      nextVisible.getMonth(),
      clampedDay,
    ...
    <div className="w-full py-10 space-y-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
        <div>
          ...
          <p className="text-muted-foreground">Clique em um dia para ver ou registrar os eventos daquela data.</p>
        </div>
        <Button
          onClick={() => {
            setEditingEvent(null);
          ...
          <CardHeader className="pb-2 border-b bg-muted/40">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" size="icon" onClick={() => handleMonthChange(-1)} className="rounded-full">
        ...
        <Card className="shadow-lg border rounded-3xl">
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <div className="text-xs text-muted-foreground">Clique em um card para editar ou remover.</div>
            </div>
            <Separator />
