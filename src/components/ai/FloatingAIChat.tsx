...
const FloatingAIChat = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: "1",
      content:
        "Pronto para ajudar com dados do SAF (licen?as, escolas, usuários, mtricas, agenda) ou melhorar um texto. O que precisa?",
      role: "assistant",
      timestamp: new Date(),
      origin: "offline",
    },
  ...
  useEffect(() => {
    setMessages((prev) => {
      ...
      const personalizedGreeting = userDisplayName
        ? `Oi, ${userDisplayName}. Dados do SAF (licen?as, escolas, usuários, mtricas, agenda) ou melhorar um texto?`
        : `Pronto para ajudar com dados do SAF (licen?as, escolas, usuários, mtricas, agenda) ou melhorar um texto.`;
  ...
  const simulateAIResponse = async (input: string): Promise<string> => {
    ...
    if (lowerInput.includes("relatrio") || lowerInput.includes("dashboard")) {
      return "Diga a mtrica (licen?as Canva, escolas, usuários, agenda) e eu mostro onde ver no SAF e como interpretar.";
    }

    if (lowerInput.includes("licen?a") || lowerInput.includes("licen?as") || lowerInput.includes("canva")) {
      return "Consigo trazer totais, usadas, disponíveis, excesso e no conformes por escola, alm de domínios autorizados e usuários vinculados. Qual dado voc quer?";
    }

    return "Atendo apenas com dados do SAF: licen?as Canva, escolas, usuários, mtricas/dashboards, agenda ou reviso de textos. Se no houver dado no sistema, aviso que no encontrei. O que voc precisa?";
  };
