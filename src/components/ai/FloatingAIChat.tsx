...
import { Bot, Send, Minimize2, Maximize2, RotateCcw, Wand2, X, BookOpenText } from "lucide-react";
...
const ORIGIN_BADGE_STYLE: Record<ResponseOrigin, string> = {
  api: "bg-primary text-primary-foreground",
  bi: "bg-info text-info-foreground",
  knowledge: "bg-warning text-warning-foreground",
  school: "bg-success text-success-foreground",
  tickets: "bg-secondary text-secondary-foreground",
...
const FloatingAIChat = () => {
  ...
  const buildTicketAnswer = (message: string): string | null => {
    ...
    if (!ticket) {
      return `Não encontrei o ticket ${ticketId} na fila atual. Confirme o ID ou cadastre o chamado.`;
    }
    ...
    return `Ticket ${ticket.id} ${statusText} com ${ticket.agente} há ${diasPendente} dia${diasPendente === 1 ? "" : "s"}${metaInfo}.${obs}`;
  };
  ...
  const buildCriticalSummaryAnswer = (message: string): string | null => {
    ...
    if (!criticalPending.length) {
      return "Nenhum ticket pendente em situação crítica (>=15 dias) neste momento.";
    }
    ...
    return `Há ${criticalPending.length} tickets pendentes em situação crítica (>=15 dias). Principais: ${topList.join("; ")}.`;
  };
  ...
  const querySchoolData = async (message: string): Promise<string | null> => {
    ...
    if (
      lowerMessage.includes("última atualização") ||
      lowerMessage.includes("ultima atualizacao") ||
      lowerMessage.includes("ltima atualizacao")
    ) {
    ...
    return `Não localizei essa escola agora. Me envie o nome completo, cidade ou cluster para tentar de novo. ${suggestionText}`;
  };
  ...
  if (
    lowerInput.includes("relatorio") ||
    lowerInput.includes("relatório") ||
    ...
    if (lowerInput.includes("licença") || lowerInput.includes("licenças") || lowerInput.includes("canva")) {
    ...
    return "Atendo apenas com dados do SAF: licenças Canva, escolas, usuários, métricas/dashboards, agenda ou reviso de textos. Se não houver dado no sistema, aviso que não encontrei. O que você precisa?";
  };
  ...
  const renderOriginBadge = (origin?: ResponseOrigin, sourceLabel?: string) => {
    if (!origin) return null;
    const label = sourceLabel || ORIGIN_LABEL[origin];
    return (
      <p
        aria-label={`Origem da resposta: ${label}`}
        className={`inline-flex items-center gap-1 text-[11px] font-semibold uppercase tracking-wide mb-1 px-2 py-1 rounded-full ${ORIGIN_BADGE_STYLE[origin]}`}
      >
    ...
    return (
      <div className="fixed bottom-6 right-6 z-50">
        <Button
          ...
          size="icon"
          className="icon-btn-touch rounded-full shadow-lg hover:shadow-xl transition-all duration-300 bg-background hover:bg-muted p-0"
          type="button"
        >
  ...
  return (
    <div
      className="fixed bottom-3 right-3 left-3 sm:left-auto sm:bottom-4 sm:right-6 z-50"
      style={{ width: "min(95vw, 360px)" }}
    >
      <Card
        className="shadow-2xl transition-all duration-300"
        style={!isMinimized ? { height: "78vh" } : { height: "3.5rem" }}
      >
          ...
          >
            ...
            >
                ...
                >
                  ...
                  >
                    {message.role === "assistant" &&
                      renderOriginBadge(message.origin, message.sourceLabel)}
              ...
              {isLoading && (
                <div className="flex justify-start" role="status" aria-live="polite">
                    <div className="bg-muted p-3 rounded-lg" aria-hidden="true">
                      <span className="sr-only">Processando mensagem</span>
            ...
            <div className="flex gap-3 items-start">
              <Input
                ...
                disabled={isLoading}
                className="flex-1 text-sm px-3 py-3 min-h-11"
              />
              ...
              >
                ...
                <span className="sr-only">Preparar melhoria</span>
              </Button>
              </div>
            </div>
          </CardContent>
