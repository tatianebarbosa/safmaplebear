...
import { clearAuthState, getCurrentSessionUser, getStoredAuthToken } from "@/lib/authSession";
  ...
  persist(
    (set, get) => ({
      ...
      canManageTicket: (ticketAgente) => {
        ...
        return role === "agent" && currentUser.agente === ticketAgente;
      },
      ...
      canAddNote: () => {
        const role = String(get().currentUser?.role || "").toLowerCase();
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        const tokenUser = getCurrentSessionUser();
        if (tokenUser) {
